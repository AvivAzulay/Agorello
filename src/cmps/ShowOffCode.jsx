...
onClickBoardMember = (member, isChecked) => {
    ...
    const card = { ...card } ///Cloning card for adding activity key
    ...
    if (!isChecked) {
        ...
        saveActivity(card, 'ADD_MEMBER')
    } else {
        members = card.members.filter(cardMember => cardMember._id !== member._id)
        // Add Activity
        card.removedMember = member
        saveActivity(card, 'REMOVE_MEMBER')
    }
